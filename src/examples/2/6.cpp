...
int main() {
    // Eigen::Matrix2d x = Eigen::Matrix2d::Random();
    // Eigen::Matrix2d y = Eigen::Matrix2d::Random();
    // std::cout << "x" << std::endl << x << std::endl << "y" << std::endl << y << std::endl;
    // std::cout << "x.dot(y) \n" << x.dot(y) << std::endl; // 不支持，错误
    // std::cout << "x.adjoint()*y \n" << x.adjoint()*y << std::endl; // 支持
    // std::cout << "x.cross(y) \n" << x.cross(y) <<std::endl; // 不支持，错误
